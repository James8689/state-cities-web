...
export function RegionMap({
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const stateSources: RegionMapData["stateSources"] = [];

        await Promise.all(
          region!.states.map(async (usps) => {
            ...
            const [stateRes, citiesRes] = await Promise.all([
              ...
              fetch(publicAssetUrl(bundle.meta.mapFiles.cities)),
            ]);
            if (!stateRes.ok || !citiesRes.ok) return;
            ...
            stateSources.push({ usps, topoUrl: bundle.meta.mapFiles.state });
  ...
  if (loadError) {
    return (
      <div className="map-container map-error" ref={ref}>
        <p>Regional map failed to load</p>
        <code>{loadError}</code>
      </div>
    );
  }
  ...
  return (
    <div className="map-container" ref={ref}>
      {!data || !projection ? (
        <div className="map-loading">Loading regional map...</div>
      ) : (
        <ComposableMap
